...
func Test_requestLogger(t *testing.T) {
	...
	if logBuffer.String() != expectedLogString {
		t.Errorf("Expected log string to be %s, but got %s", expectedLogString, logBuffer.String())
	}
	if rr.Code != expectedStatusCode {
		t.Errorf("Expected status code to be %d, but got %d", expectedStatusCode, rr.Code)
	}
